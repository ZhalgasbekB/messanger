...
func (h *Handler) InitRouters() http.Handler {
	...
	// websocket
	mux.Handle("/conversations", h.authUser(http.HandlerFunc(h.WebSocketHandler.Conversations)))
	mux.Handle("/conversation", h.authUser(http.HandlerFunc(h.WebSocketHandler.Conversation))) // PARAMETER ID
